...
MECHANICAL_ENERGY_OVEN_PROPORTION = (
    0.069552  # Mechanical Energy oven proportion
)
GYPSUM_PROPORTION = 0.04  # Gypsum proportion
MECHANICAL_ENERGY_MILLING_PROPORTION = (
    0.135792  # Mechanical Energy used by the process
)
...
class Cement:
    ...
    def __init__(self, total_cement_production):
        ...
        self.__limestone_demand = (
            self.__total_cement_production * LIMESTONE_PROPORTION
        )
        self.__clay_demand = self.__total_cement_production * CLAY_PROPORTION
        self.__mechanical_energy_pre = (
            self.__total_cement_production * MECHANICAL_ENERGY_PRE_PROPORTION
        )
        self.__pre_homogeneization_and_grinding(
            self.__limestone_demand, self.__clay_demand
        )
        ...
        self.__mechanical_energy_oven = (
            self.__total_cement_production * MECHANICAL_ENERGY_OVEN_PROPORTION
        )
        self.__oven(
            self.__pm10_emission_pre, self.__fuel_demand, self.__raw_mix
        )
        self.__gypsum_demand = (
            self.__clinker_production_oven * GYPSUM_PROPORTION
        )
        self.__mechanical_energy_milling = (
            self.__total_cement_production
            * MECHANICAL_ENERGY_MILLING_PROPORTION
        )
        self.__milling(self.__clinker_production_oven, self.__gypsum_demand)
        self.__mechanical_energy = (
            self.__mechanical_energy_pre
            + self.__mechanical_energy_oven
            + self.__mechanical_energy_milling
        )
        self.__co2_overall_emissions = (
            total_cement_production * CO2_EMISSIONS_PROPORTION
        )
        self.__heat_overall_losses = self.__heat_losses_oven
        self.__pm10_overall_emission = (
            self.__pm10_emission_pre
            + self.__pm10_emission_oven
            + self.__cement_emission
        )

    def __validate_total_production(self, total_cement_production) -> None:
        if (
            total_cement_production < 0
            or total_cement_production > 100000000000000
        ):
            raise ValueError(
                "The production should be a value between 5 and 1000"
            )

    def __pre_homogeneization_and_grinding(
        self, limestone_demand, clay_demand
    ) -> None:
        """Pre-homogenization and grinding handling of raw materials"""
        self.__pm10_emission_pre = (
            CLAY_LOSSES * clay_demand + LIMESTONE_LOSSES * limestone_demand
        )
        self.__raw_mix = (
            clay_demand + limestone_demand - self.__pm10_emission_pre
        )
    ...
    def __milling(self, clinker_production_oven, gypsum_demand) -> None:
        """Milling"""
        self.__cement_emission = CEMENT_LOSSES * (
            clinker_production_oven + gypsum_demand
        )
        self.__cement_production = (1 - CEMENT_LOSSES) * (
            clinker_production_oven + gypsum_demand
        )
