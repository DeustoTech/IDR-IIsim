...
from idr_iisim.utils.config      import GlobalVariables  # read config file and set global variables
...
def find_processes() -> None:
    ...
    for root, dirs, files in os.walk(path, topdown=False):      # root: The current directory path, dirs: List of subdirectories, files: List of files in the current directory.
...
def init() -> None:
    ...
    i_logger.logger.debug("yaml validator initialized (based on a schema.yaml)\n")
    ...
    for key, value in models_dict.models.items():
        ...
        for model_instance_id in queue:
            ...
            if model_instance_id in all_dependencies:
                ...
                i_logger.logger.debug(f"checking model dependencies: {dependencies}")
